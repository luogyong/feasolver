...

{
  ...
  //the first attributes is assumed to be the size info.
  osize = triorg[2];
  dsize = tridest[2];
  asize = triapex[2];
  /* Find the squares of the lengths of the triangle's three edges. */
  oalen = sqrt(dxoa * dxoa + dyoa * dyoa);
  dalen = sqrt(dxda * dxda + dyda * dyda);
  odlen = sqrt(dxod * dxod + dyod * dyod);
...

{
  ...
  int i;
  enum locateresult intersect;
  double *c1,tri1[6],p1[2];
  vertex borg1, bdest1, bapex1;
  ...
  if (!deadtri(badotri.tri) && (borg == badtri->triangorg) &&
    ...
    if (((newvertex[0] == borg[0]) && (newvertex[1] == borg[1])) ||
      ...
      vertexdealloc(m, newvertex);
        ...
        if(b->usertest && ((xi<0.0||eta<0.0)||(1.-xi-eta<0.0))){
          if (b->verbose > 2) {
            printf("Entering the code by LGY-Try to Locate VERTEX IS: (%.12g, %.12g)\n", newvertex[0], newvertex[1]);
          }
          decode(badtri->poortri, badotri1);
          intersect=locate(m,b,newvertex,&badotri1);
          if (b->verbose > 2){
            printf("LGY-NEWVERTEX IS: (%.12g, %.12g)\n", newvertex[0], newvertex[1]);
            printf("LGY-Locate result %d:\n",(int)intersect);
          }
          if(intersect!=OUTSIDE){
            org(badotri1, borg1);
            dest(badotri1, bdest1);
            apex(badotri1, bapex1);
            tri1[0]=borg1[0];
            tri1[1]=borg1[1];
            tri1[2]=bdest1[0];
            tri1[3]=bdest1[1];
            ...
            p1[0]=newvertex[0];
            p1[1]=newvertex[1];                    
            c1 = triangle_barycentric_2d(tri1, p1);  
            if (b->verbose > 2){          
              printf("LGY-TRI IS: (%.12g, %.12g) (%.12g, %.12g) (%.12g, %.12g)\n", borg1[0],
                  borg1[1], bdest1[0], bdest1[1], bapex1[0], bapex1[1]);            
              printf("LGY-Barycentric Coordinates IS: (%.12g, %.12g, %0.12g)\n", c1[0], c1[1],c1[2]);
              //getchar();
            }
            for (i = 2; i < 2 + m->nextras; i++) {
              /* Interpolate the vertex attributes at the circumcenter. */
              newvertex[i] = borg1[i]*c1[2] + bdest1[i]*c1[0]+bapex1[i]*c1[1] ;
            }
          }
          else{
            for (i = 2; i < 2 + m->nextras; i++) {
