module maximum_inscribed_circle
    ...
    type gpolygon_tydef
        ...
        integer::htype=2
        !当cell为tri or tet时, htype=0,其特征长度为形心到各节点距离的最大值;=1,!单元特征长度为形心到节点距离减去节点半径的最大值;=2,boundingbox的对角线长度的一半.
    ...
    function maximum_distancetopolygon(self,precision,debug,start,maxiter) result(ar)
        ...
        !allocate(ar(self.dim+1+size(self.dist)))
    ...
    subroutine tet_decomposition(tri,x,node,p,h,isdiv)
        ...
        if(size(tri,dim=2)==3) then
            ...
            node(:,3)=[4,2,5]
            node(:,4)=[6,5,3]
            n1=4;n2=3;n3=3 !nsubcell,nnode,nedge
        else
            ...
            node(:,7)=[7,10,8,9]
            node(:,8)=[5,7,8,9]            
            n1=8;n2=4;n3=6 !nsubcell,nnode,nedge
            istri=.false.
        endif
        ...
        do i=1,n1
            ...
            if(n2==4.or.i==1) then                
                h(i)=characteristiclength(x(:,node(:,i)))
    ...
    real(8) function characteristiclength(v)
        implicit none
        real(8),intent(in)::v(:,:) ![dim+1,nnode] 
        real(8)::center(3),box(6) !box=[xmin,ymin,zmin,xmax,ymax,zmax]
        
        
        
        center=sum(v(1:self.dim,:),dim=2)/(self.dim+1)

        if(self.htype==1) then
            !单元特征长度h(为形心到节点距离减去节点半径的最大值）
            characteristiclength=maxval(norm2(v(:self.dim,:)-spread(center,2,self.dim+1),dim=1)-v(self.dim+1,:))
        elseif(self.htype==0) then
            !单元特征长度h(为形心到各节点距离的最大值）
            characteristiclength=maxval(norm2(v(:self.dim,:)-spread(center,2,self.dim+1),dim=1))    
        else
            !bounding box 对角线长度的一半
            box(1:3)=minval(v(1:3,:),dim=2)
            box(4:6)=maxval(v(1:3,:),dim=2)
            characteristiclength=0.5*norm2(box(4:6)-box(1:3))                
        endif
        ...
        associate (x => this%buf)
            ...
            do while(parent*2 <= this%n)
                ...
                if (x(parent)%priority < x(child)%priority) then
                    ...
                    cell1=x(parent);x(parent)=x(child);x(child)=cell1                   
                    parent = child
                else
                    exit
                end if  
