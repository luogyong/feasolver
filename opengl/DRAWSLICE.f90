...
SUBROUTINE GEN_SLICE_SURFACE_DATA()
    ...
    real(8)::SLICEVAL1(NEDGE,POSDATA.NVAR),SLICEPOINT1(3,NEDGE)
    INTEGER::NTRI1
    ...
    subroutine GENSLICE(AXIS,X,VAL,SLICEPOINT,TRI,NTRI)
        ...
        real(8),intent(out)::SLICEPOINT(3,NEDGE),VAL(NEDGE,POSDATA.NVAR)    
    end subroutine  
    ...
    DO ISLICE=1,NSLICE
        ...
        ALLOCATE(SLICE(ISLICE).V(3,SLICE(ISLICE).NV),SLICE(ISLICE).VAL(SLICE(ISLICE).NV,POSDATA.NVAR))
...
SUBROUTINE GEN_SLICE_ISOLINE_DATA()
   ...
   ! ENDSUBROUTINE
    ...
    DO ISLICE=1,NSLICE
        ...
        CALL SETUP_EDGE_ADJL(EDGE1,NEDGE1,ELT1,SLICE(ISLICE).NTRI,SLICE(ISLICE).V,SLICE(ISLICE).NV,POSDATA.ESET,POSDATA.NESET)
        CALL SETUP_FACE_ADJL(FACE1,NFACE1,EDGE1,NEDGE1,ELT1,SLICE(ISLICE).NTRI,SLICE(ISLICE).V,SLICE(ISLICE).NV,POSDATA.ESET,POSDATA.NESET) 
		...
		IF(CONTOURBAR.IVARPLOT/=SLICE_PLOT_VARIABLE) CALL initialize_contourplot(SLICE_PLOT_VARIABLE)        
...
subroutine GENSLICE(AXIS,X,VAL,CONTOURPOINT,TRI,NTRI)
    ...
    integer,intent(out)::NTRI
    INTEGER,allocatable,intent(out)::TRI(:,:)
    real(8),intent(out)::CONTOURPOINT(3,NEDGE),VAL(NEDGE,POSDATA.NVAR)
    ...
    REAL(GLDOUBLE)::SCALE1,X1(3),X2(3) 
    ...
    do i=1,nedge
        ...
        X1=POSDATA.NODE(EDGE(I).V(1)).COORD+VEC1(:,EDGE(I).V(1))*SCALE1
		X2=POSDATA.NODE(EDGE(I).V(2)).COORD+VEC1(:,EDGE(I).V(2))*SCALE1
        ...
        ELSE
            IF(T1*T2<=0) THEN
                ...
                DO J=1,POSDATA.NVAR
					V1=POSDATA.NODALQ(EDGE(I).V(1),J,STEPPLOT.ISTEP);V2=POSDATA.NODALQ(EDGE(I).V(2),J,STEPPLOT.ISTEP);
					VAL(I,J)=V1+t4*(V2-V1)
                ENDDO
            ENDIF   
        ENDIF 
