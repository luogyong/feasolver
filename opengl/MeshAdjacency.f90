...
SUBROUTINE SETUP_FACE_BC()
    IMPLICIT NONE
    INTEGER::I,J,N1,N2,N3
    INTEGER,ALLOCATABLE::IA1(:)
    
    NF_BC=COUNT(FACE.ENUM==1)
    ALLOCATE(FACE_BC(NF_BC),IA1(POSDATA.NNODE))
    NF_BC=0;IA1=0
    DO I=1,NFACE
        IF(FACE(I).ENUM==1) THEN
            NF_BC=NF_BC+1
            FACE_BC(NF_BC)=I
            IA1(FACE(I).V(1:FACE(I).SHAPE))=FACE(I).V(1:FACE(I).SHAPE)
            ! DO J=1,2
            !     N1=MINLOC(IA1(:,FACE(I).V(J)),MASK=IA1(:,FACE(I).V(J))==0,DIM=1)
            !     IF(N1==0) THEN
            !         STOP "SIMPLE LOOP IS ASSUMED"
            !     ENDIF
            !     IA1(N1,FACE(I).V(J))=FACE(I).V(MOD(J,2)+1)
            ! ENDDO
        ENDIF
    ENDDO

    NODE_LOOP_BC=PACK(IA1,IA1>0)
    NNLBC=SIZE(NODE_LOOP_BC)
    ! N1=FACE(FACE_BC(1)).V(1)
    ! N2=FACE(FACE_BC(1)).V(2)
    ! NODE_LOOP_BC(1:2)=[N1,N2]
    ! DO I=2,NF_BC
    !     N2=NODE_LOOP_BC(I);N1=NODE_LOOP_BC(I-1)        
    !     N3=MINLOC(IA1(:,N2)-N1,MASK=ABS(IA1(:,N2)-N1)>0,DIM=1)
...
SUBROUTINE SETUP_NODE2ELEMENT_ADJL_SOLVER(NADJL)
    USE solverds,ONLY:ELEMENT_L=>ELEMENT,NODE_L=>NODE,NNODE_L=>NNUM,NEL_L=>ENUM,ESET=>eset
    IMPLICIT NONE
    TYPE(NODE_ADJ_TYDEF),ALLOCATABLE::NADJL(:)
    INTEGER::ET1,N1,V1(2)
    INTEGER::I,J,ISET1
    ...
    IF(.NOT.ALLOCATED(NADJL)) ALLOCATE(NADJL(NNODE_L))    
    NADJL.ENUM=0
	DO I=1,NEL_L
        ISET1=ELEMENT_L(I).SET
        IF(ESET(ISET1).COUPLESET>0 &            
    ...
    DO I=1, NNODE_L
        IF(ALLOCATED(NADJL(I).ELEMENT)) DEALLOCATE(NADJL(I).ELEMENT)
        IF(ALLOCATED(NADJL(I).SUBID)) DEALLOCATE(NADJL(I).SUBID)        
        ALLOCATE(NADJL(I).ELEMENT(NADJL(I).ENUM),NADJL(I).SUBID(NADJL(I).ENUM))        
        NADJL(I).ENUM=0
    ENDDO
