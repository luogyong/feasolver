...
SUBROUTINE ShowLocalMinimalSlope()
    ...
    DO I=1,NSTREAMLINE
        RA1(I)=STREAMLINE(I).VAL(POSDATA.IX,1)
    ENDDO
...
subroutine streamline_integration(istreamline)
    ...
    DO J=1,2
        ...
        ELSE
            ...
            !边界点的定位可能出问题
            IF(STREAMLINE(istreamline).NV>2) THEN
                 IF(STREAMLINE(ISTREAMLINE).IEL(1)==0)  STREAMLINE(ISTREAMLINE).IEL(1)=STREAMLINE(ISTREAMLINE).IEL(2) !
                IF(STREAMLINE(ISTREAMLINE).IEL(STREAMLINE(istreamline).NV)==0)  STREAMLINE(ISTREAMLINE).IEL(STREAMLINE(istreamline).NV)=STREAMLINE(ISTREAMLINE).IEL(STREAMLINE(istreamline).NV-1) !
...
SUBROUTINE GET_SEGINTERCECTION(P1,P2,T1,T2,IPT1,IPT2,ISINTERCEPT,DIM)
    USE MESHGEO,ONLY:ISFRONT,ISACW 
    IMPLICIT NONE
