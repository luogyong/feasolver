...
INTEGER::NMEDGE=0,NMFACE=0,MAXNMEDGE=10000,MAXNMFACE=10000 !M FOR MODEL 
...
INTEGER,ALLOCATABLE::EDGE_BC(:),FACE_BC(:)
INTEGER::NE_BC=0,NF_BC=0
	...
	DO I=1,ENUM
		...
		DO J=1,NFACE1
            ...
            IF(.NOT.MFACE(VAL1.IITEM).ISINI) THEN
                ...
                NMFACE=VAL1.IITEM
            ENDIF
            IF(MFACE(VAL1.IITEM).ENUM==0) ALLOCATE(MFACE(VAL1.IITEM).ELEMENT(2))
            MFACE(VAL1.IITEM).ENUM=MFACE(VAL1.IITEM).ENUM+1
            IF(MFACE(VAL1.IITEM).ENUM>SIZE(MFACE(VAL1.IITEM).ELEMENT,DIM=1)) CALL I_ENLARGE_AR(MFACE(VAL1.IITEM).ELEMENT,5)
	...
	DO I=1,NEL_L
		...
		DO J=1,NFACE1
            ...
            IF(FACE_L(VAL1.IITEM).ENUM>1) THEN
                DO K=1,FACE_L(VAL1.IITEM).SHAPE
                    IF(FACE_L(VAL1.IITEM).V(K)==ELEMENT_L(I).V(ELTTYPE(ELEMENT_L(I).GMET).FACE(1,J))) THEN
                        IF(FACE_L(VAL1.IITEM).V(MOD(K,FACE_L(VAL1.IITEM).SHAPE)+1)/= &
                            ...
                            FACE_L(VAL1.IITEM).ELEMENT(FACE_L(VAL1.IITEM).ENUM)=-I !I单元此面的方向与face的方向反向,为-1
                        ELSE
                ...
                ENDDO
            ELSE
                FACE_L(VAL1.IITEM).ELEMENT(FACE_L(VAL1.IITEM).ENUM)=I !同向 =1
            ENDIF
