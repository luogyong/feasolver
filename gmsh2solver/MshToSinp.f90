module DS_Gmsh2Solver
	...
	type physicalGroup_type
        ...
        INTEGER,ALLOCATABLE::TRISURFACE(:),QUASURFACE(:),TRINODEG2L(:),TRINODEL2G(:)
	...
	DO I=1,NEL
		...
		DO J=1,NEDGE1
            ...
            IF(.NOT.EDGE(VAL1.IITEM).ISINI) THEN
                ...
                NEDGE=VAL1.IITEM
            ENDIF
            IF(EDGE(VAL1.IITEM).NEL==0) ALLOCATE(EDGE(VAL1.IITEM).ELEMENT(5))
            EDGE(VAL1.IITEM).NEL=EDGE(VAL1.IITEM).NEL+1
            IF(EDGE(VAL1.IITEM).NEL>SIZE(EDGE(VAL1.IITEM).ELEMENT,DIM=1)) CALL I_ENLARGE_AR(EDGE(VAL1.IITEM).ELEMENT,5)
            ...
            DO CONCURRENT (J=1:NFACE)
                IF(ABS(FACE(J).ISTRISURFACE)==IGP1) THEN                    
					...
					ELSE
						...
						PHYSICALGROUP(IGP1).QUASURFACE(PHYSICALGROUP(IGP1).NQUASURFACE)=J
                    ENDIF
                ENDIF                    
			ENDDO
...
subroutine write_readme_gmsh2sinp()	
    ...
    README(IPP(I)) = "//A:{OUTPUTFORMAT}    //1:*.GEO;2:*.GTS."  
