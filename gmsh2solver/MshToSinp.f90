module DS_Gmsh2Solver
	...
	character(512)::resultfile,title,INCLUDEFILE(0:100),COPYFILE(100),HELPFILE
...
10 format("Press 'H' to write a keyword help file named GMSH2SOLVER_HELP.TXT. Any other key to read an Msh file.")		
end program
...
subroutine write_readme_gmsh2sinp()	
	...
	README(IPP(I)) = "//A:{INODE,X,Y,Z(NLAYER+1)}*{NNODE}  // 共3+NLAYER+1个/行.Z IS IN A2Z ORDER(Z1<=Z2<=...<=Z(NLYAER+1)),共NNODE行."
...
subroutine readin()
	...
	DO WHILE(I<=NINCLUDEFILE)
		...
		IF(I==0) THEN
			...
			HELPFILE=trim(drive)//trim(dir)//'GMSH2SOLVER_HELP.TXT'
		ELSE
