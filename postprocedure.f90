...
subroutine pointout(FILE_UNIT,ISTEP,ISUBTS,ITER)
	...
	do while(i<=nvo)
		select case(vo(i))
			...
			case(SFR)
				do j=1,12
					NodalQ(:,i+j-1,nnodalq)=GET_VALUE_NODE(node,[1:nnum],'sfr',j)
				enddo
			CASE(PSIGMA)
				do j=1,13
					DO K=1,NNUM 
...
subroutine out_datapoint(ISTEP,ISUBTS,ITER,IDISQ1)
	...
	IF(NDATAPOINT>0) THEN
		...
		DO I=1,NDATAPOINT		
			IF (DATAPOINT(I).ISSUMQ==0) THEN
				DO J=1,DATAPOINT(I).NNODE
					write(DATAPOINT_UNIT,110) SOLVER_CONTROL.ISPARASYS,SOLVER_CONTROL.CaseID,I,ISTEP,ISUBTS,ITER,J,NDOF,(NodalQ(DATAPOINT(I).NODE(J),K,nnodalq),K=1,nvo)
				END DO
			ELSE
				SUMQ1=0.0				
				DO J=1,DATAPOINT(I).NNODE					
				...
				END DO
				NVO1=1
				WRITE(DATAPOINT_UNIT,120) SOLVER_CONTROL.ISPARASYS,SOLVER_CONTROL.CaseID,I,ISTEP,ISUBTS,ITER,J,NDOF,SUMQ1,'SUMQ'
...
999 format(<nvo>(E24.15,1X))
100	FORMAT("IPARASYS",7X,"CaseID",9X,"DATASET",8X,"ISTEP",10X,"ISUBTS",9X,"ITER",11X,"NO",13X,"NDOF",11X,<NVO>A15)
110 FORMAT(8(I14,X),<nvo>(E14.7,X))
120 FORMAT(8(I14,X),<NVO1>(E14.7,X),"//",<NVO1>A15)
130 FORMAT("IPARASYS",7X,"CaseID",9X,"DATASET",8X,"ISTEP",10X,"ISUBTS",9X,"ITER",11X,<16>(A14,X))
131 FORMAT(6(I14,X),A14,X,15(E14.7,X))   
...
subroutine BlOCKout(file_unit,ISTEP,ISUBTS,ITER)
	...
	do while(i<=nvo)
		select case(vo(i))
			...
			case(sxx)
			  write(file_unit,999)  GET_VALUE_NODE(node,[1:nnum],'s',1)
			case(syy)
			  write(file_unit,999)  GET_VALUE_NODE(node,[1:nnum],'s',2)
			case(szz)
			  write(file_unit,999)  GET_VALUE_NODE(node,[1:nnum],'s',3)
			case(sxy)
				write(file_unit,999)  GET_VALUE_NODE(node,[1:nnum],'s',4)	
			case(syz)
				write(file_unit,999)  GET_VALUE_NODE(node,[1:nnum],'s',5)
			case(szx)
				write(file_unit,999)  GET_VALUE_NODE(node,[1:nnum],'s',6)
			case(exx)
				write(file_unit,999)  GET_VALUE_NODE(node,[1:nnum],'e',1)
			case(eyy)
				write(file_unit,999)  GET_VALUE_NODE(node,[1:nnum],'e',2)
			case(ezz)
				write(file_unit,999)  GET_VALUE_NODE(node,[1:nnum],'e',3)
			case(exy)
				write(file_unit,999)  GET_VALUE_NODE(node,[1:nnum],'e',4)
			case(eyz)
				write(file_unit,999)  GET_VALUE_NODE(node,[1:nnum],'e',5)
			case(ezx)
			...
			case(gradx)
				write(file_unit,999) GET_VALUE_NODE(node,[1:nnum],'i',1)		
			case(grady)
				write(file_unit,999) GET_VALUE_NODE(node,[1:nnum],'i',2)	
			case(gradz)
				write(file_unit,999) GET_VALUE_NODE(node,[1:nnum],'i',3)
			case(vx)
				write(file_unit,999) GET_VALUE_NODE(node,[1:nnum],'v',1)
			case(vy)
				write(file_unit,999) GET_VALUE_NODE(node,[1:nnum],'v',2)
			case(vz)
				write(file_unit,999) GET_VALUE_NODE(node,[1:nnum],'v',3)
			case(discharge)
			...
			case(PSIGMA)
				do j=1,13
					write(file_unit,999) GET_VALUE_NODE(node,[1:nnum],'ps',j)
				enddo				
			case(NF)
				do j=1,NDIMENSION
            ...
            case(throatQ)
                write(file_unit,999)  abs(GET_VALUE_ELEMENT(element,[1:ENUM],'flux',1))
            case(throatFriction)
                write(file_unit,999)  1.0/GET_VALUE_ELEMENT(element,[1:ENUM],'km',1,1)
            case(PF_LEN_CLOGGING)
                write(file_unit,999)  GET_VALUE_ELEMENT(element,[1:ENUM],'pfp',6)+GET_VALUE_ELEMENT(element,[1:ENUM],'pfp',7)
			case(PF_PC)         
                write(file_unit,100)  int(GET_VALUE_ELEMENT(element,[1:ENUM],'pfp',8)+GET_VALUE_ELEMENT(element,[1:ENUM],'pfp',9))
			case(THROATVOL)         
                write(file_unit,999)  GET_VALUE_ELEMENT(element,[1:ENUM],'pfp',4)+GET_VALUE_ELEMENT(element,[1:ENUM],'pfp',5)
			case(ELT_ID)         
