SUBROUTINE SPG_Q_UPDATE(STEPDIS,bload,HHEAD,INIHEAD,DT,nbload,ienum,iiter,istep,iscon)
    ...
    CASE(ZT4_SPG,ZT6_SPG)
        ...
        BLOAD=ELEMENT(IENUM).FLUX
    CASE(POREFLOW) !
        CALL POREFLOW_Q_K_UPDATE(STEPDIS,IENUM,ISTEP,IITER)
        !CALL POREFLOW_Q_K_UPDATE(STEPDIS,IENUM,ISTEP,IITER)
	    !IF(MINVAL(HHEAD-NODE(ELEMENT(IENUM).NODE(1:nbload)).COORD(NDIMENSION))>-1.D-6) THEN
     !       t1=1.0D0 !承压
     !   ELSE
	    !    t1=1.D-3 !某一节点在非饱和区                 						
     !   ENDIF                
     !   ELEMENT(IENUM).FLUX=(ELEMENT(IENUM).KR(1)+t1)/2.0*matmul(ELEMENT(IENUM).KM,HHEAD(1:NBLOAD))
     !   ELEMENT(IENUM).KR(1)=t1
     !   BLOAD=ELEMENT(IENUM).FLUX
     !   !计算雷诺数
     !   A1=element(IENUM).property(2) !喉径
     !   A2=3.1415926*A1**2/4.0 !面积
     !   !QA1=ABS((NHEAD1(1)-NHEAD1(2))*ELEMENT(IEL).KM(1,1))
     !   T1=ABS(ELEMENT(IENUM).FLUX(1))/A2 !流速
     !   ELEMENT(IENUM).PROPERTY(3)=Re_W(T1,A1,MATERIAL(ELEMENT(IENUM).MAT).PROPERTY(2))  
